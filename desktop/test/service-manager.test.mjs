...
test('absolute Docker.app CLI path is passed to launcher and stop', async()=>{let spawnOptions;let stopCommand;const prerequisites=async()=>({dockerCli:true,dockerDaemon:true,dockerCliPath:'/Applications/Docker.app/Contents/Resources/bin/docker'});const m=base({prerequisites,discover:async()=>[],spawnProcess:(_command,_args,options)=>{spawnOptions=options;return {stdout:{on(){}},stderr:{on(){}}}},run:async(command)=>{stopCommand=command;},timeouts:{startup:5}});await Promise.allSettled([m.startField()]);assert.equal(spawnOptions.env.SURFJUDGING_DOCKER_BIN,'/Applications/Docker.app/Contents/Resources/bin/docker');await m.stopField({confirmed:true});assert.equal(stopCommand,'/Applications/Docker.app/Contents/Resources/bin/docker');});
