...
const { makeManager } = require('../src/main/field-service-manager.cjs');
...
test('stop requires explicit confirmation when safe', async()=>{const m=base({run:async(cmd,args)=>{assert.equal(cmd,'docker'); assert.deepEqual(args.slice(0,2),['stop','surfjudging']);}}); const r=await m.stopField(); assert.equal(r.result,'CONFIRMATION_REQUIRED'); assert.equal(m.getState(),'STOP_CHECK');});
