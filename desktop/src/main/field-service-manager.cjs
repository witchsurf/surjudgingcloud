...
function makeManager({ rootDir, platform = process.platform, discover, health, prerequisites, fetchRunningHeats, run = execFileAsync, spawnProcess = spawn, now = () => new Date().toISOString(), timeouts = {} }) {
  ...
  const checkRuntime = async () => { setState('CHECKING_RUNTIME'); const p = await prerequisites(); if (!p.dockerCli || !p.dockerDaemon) { setState('RUNTIME_UNAVAILABLE'); return { ok:false, state, prerequisites:p }; } return { ok:true, state, prerequisites:p }; };
