...
function makeManager({ rootDir, discover, health, prerequisites, fetchRunningHeats, run = execFileAsync, spawnProcess = spawn, now = () => new Date().toISOString(), timeouts = {} }) {
  let state = 'STOPPED'; let activeStart = null; let child = null; let pollTimer = null; let healthSnapshot = null; const logs = [];
  ...
  const checkRuntime = async () => { setState('CHECKING_RUNTIME'); const p = await prerequisites(); if (!p.dockerCli || !p.dockerDaemon || !p.colima) { setState('RUNTIME_UNAVAILABLE'); return { ok:false, state, prerequisites:p }; } return { ok:true, state, prerequisites:p }; };
  ...
  const stopField = async ({ confirmed = false } = {}) => { const gate = await canStopField(); if (!gate.allowed) return { result:'DENIED_RUNNING_HEAT', ...gate }; if (!confirmed) return { result:'CONFIRMATION_REQUIRED', ...gate }; setState('STOPPING'); stopPolling(); await run('docker', ['stop', ...SERVICES], { cwd:rootDir, timeout:timeout('stop',30000) }); child = null; setState('STOPPED'); return { result:'STOPPED' }; };
...
module.exports = { STATES, SERVICES, makeManager };
