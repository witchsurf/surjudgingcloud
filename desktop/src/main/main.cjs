...
async function prereq(){const command=async(n,a)=>{try{await execFileAsync(n,a,{timeout:1500});return true}catch{return false}};const host=await runtimePreparation.inspect();return {dockerCli:host.dockerCli,dockerCliPath:host.dockerCliPath,dockerDesktop:host.dockerAppInstalled,colima:await command('colima',['version']),dockerDaemon:host.dockerDaemon,hostVersion:host.hostVersion,hostArch:host.arch,preparationState:host.state}}
...
const manager=makeManager({rootDir:fieldRuntimeRoot,platform:process.platform,discover,health,prerequisites:prereq,fetchRunningHeats:runningHeats});
...
ipcMain.handle('runtime-preparation:inspect',()=>runtimePreparation.inspect());
...
app.whenReady().then(()=>{createWindow();app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow()})});app.on('before-quit',()=>livePublicationSupervisor.stop());app.on('window-all-closed',()=>{if(process.platform!=='darwin')app.quit()});
