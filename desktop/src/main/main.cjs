...
async function startFieldOnLaunch(){if(appTranslocated)return;const runtime=await manager.checkRuntime();if(!runtime.ok)return;try{await manager.startField();await livePublicationSupervisor.start();}catch(error){console.error(`FIELD_AUTO_START_FAILED ${error.message}`);}}
