const { app, BrowserWindow, clipboard, dialog, ipcMain, nativeImage, powerSaveBlocker, safeStorage, screen, shell } = require('electron');
...
const { makeOfflineLicenseService } = require('./offline-license-service.cjs');
const licensePolicy = require('../shared/license-policy.json');
...
const offlineLicenseService = makeOfflineLicenseService({ root: path.join(dataRoot(), 'licensing'), policy: licensePolicy, safeStorage, appVersion: app.getVersion() });
...
async function startFieldOnLaunch(){if(appTranslocated)return;const runtime=await manager.checkRuntime();if(!runtime.ok)return;const license=await offlineLicenseService.status();if(!license.allowed){console.error(`FIELD_AUTO_START_BLOCKED_LICENSE ${license.state}`);return;}try{await manager.startField();await livePublicationSupervisor.start();}catch(error){console.error(`FIELD_AUTO_START_FAILED ${error.message}`);}}
...
app.whenReady().then(()=>{app.setAboutPanelOptions({applicationName:'SurfJudging Field',applicationVersion:app.getVersion(),copyright:'Copyright © 2026 René Pierre LARAISE',credits:'Conçu et développé par René Pierre LARAISE\nrplaraise@gmail.com'});createWindow();void startFieldOnLaunch();app.on('activate',()=>{if(BrowserWindow.getAllWindows().length===0)createWindow()})});app.on('before-quit',()=>{livePublicationSupervisor.stop();priorityDisplayController.dispose();});app.on('window-all-closed',()=>{if(process.platform!=='darwin')app.quit()});
