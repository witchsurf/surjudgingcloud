...
function renderPreparation() { const copy=preparationText(state.preparation); $('preparation-summary').textContent=copy.summary; $('preparation-detail').textContent=copy.detail; $('prepare').textContent=state.preparing?'Préparation en cours…':copy.action; $('prepare').disabled=state.preparing || ['READY','APP_TRANSLOCATED'].includes(state.preparation?.state); }
...
async function prepareMachine(){
  ...
  if(['UNSUPPORTED','DISK_INSUFFICIENT','APP_TRANSLOCATED'].includes(state.preparation.state)){ $('status').textContent=state.preparation.state==='DISK_INSUFFICIENT'?'Préparation bloquée — libérez au moins 20 Go sur le disque.':state.preparation.state==='APP_TRANSLOCATED'?'Déplacez SurfJudging Field dans Applications, puis rouvrez-le.':'Préparation bloquée — cette version de macOS n’a pas encore de profil approuvé.'; return; }
