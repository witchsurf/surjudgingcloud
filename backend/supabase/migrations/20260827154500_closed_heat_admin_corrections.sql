...
declare
  v_status text;
begin
  -- Only the audited SECURITY DEFINER correction RPC sets this transaction-
  -- local flag. Ordinary judge inserts/updates remain blocked after closure.
  if current_setting('surfjudging.allow_closed_score_correction', true) = 'on' then
    return new;
  end if;
  ...
  if v_status = 'closed' or v_status is null then
    raise exception 'Saisie bloquée : heat clos ou non configuré (status: %)', coalesce(v_status, 'inconnu')
      using errcode = 'P0001';
  end if;
...
begin
  ...
  perform set_config('surfjudging.allow_closed_score_correction', 'on', true);
...
values (true, '20260827154500_closed_heat_admin_corrections', 'Audited Admin corrections on closed heats', now())
