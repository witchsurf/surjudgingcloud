...
drop trigger if exists trg_enrich_score_audit_from_override on public.score_overrides;
create trigger trg_enrich_score_audit_from_override
after insert or update on public.score_overrides
for each row execute function public.fn_enrich_score_audit_from_override();
...
begin
  if to_regclass('public.app_runtime_schema_version') is not null then
    ...
    values (true, '20260727192000_enrich_score_audit_actor', now())
