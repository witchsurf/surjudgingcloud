...
-- Preserve the certified score-only implementation as a non-recursive primitive.
alter function public.fn_rank_heat_entries_from_scores(text)
  rename to fn_rank_heat_entries_scored_only;

-- Exhaustive ranking starts from every legitimate heat entry, then overlays
-- the existing score/interference result. Previous-round totals are resolved
-- one lineage edge through heat_slot_mappings using the score-only primitive;
-- this avoids qualification/ranking recursion.
...
as $$
  with entries as (
    ...
    where e.heat_id = trim(p_heat_id)
  ...
  lineage as (
    select
      e.position,
      previous.best_two as previous_round_total
    from entries e
    left join public.heat_slot_mappings mapping
      on mapping.heat_id = e.heat_id
     and mapping.position = e.position
    left join public.heats source_heat
      on source_heat.event_id = (select h.event_id from public.heats h where h.id = trim(p_heat_id))
  ...
  candidates as (
    select
      ...
      (scored.best_two is not null) as has_current_score,
      lineage.previous_round_total
    from entries e
    left join scored
      on public.fn_normalize_jersey_label_sql(scored.color)
       = public.fn_normalize_jersey_label_sql(e.color)
  ...
  ranked as (
    select
      ...
      dense_rank() over (
        order by
          candidates.current_total desc,
          candidates.previous_round_total desc nulls last,
          candidates.seed asc nulls last,
  ...
  from ranked
  order by computed_rank, position;
$$;
...
grant execute on function public.fn_rank_heat_entries_scored_only(text)
  to anon, authenticated, service_role;
grant execute on function public.fn_rank_heat_entries_exhaustive(text)
  to anon, authenticated, service_role;
grant execute on function public.fn_rank_heat_entries_from_scores(text)
  to anon, authenticated, service_role;
...
begin
  if to_regclass('public.app_runtime_schema_version') is not null then
    ...
    values (true, '20260814210000_exhaustive_heat_ranking_inherited_tiebreak', now())
