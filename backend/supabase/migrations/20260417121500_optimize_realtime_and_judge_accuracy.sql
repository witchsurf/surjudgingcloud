...
-- Speed up score timeline reads used by live screens and admin views.
create index if not exists idx_scores_heat_id_created_at
  on public.scores (heat_id, created_at);

-- Speed up judge accuracy rollups by matching the normalized scorer/wave keys
-- used inside the reporting view.
create index if not exists idx_scores_event_heat_wave_surfer_judge
  on public.scores (
    event_id,
    heat_id,
    upper(trim(surfer)),
