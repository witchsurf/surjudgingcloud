...
const PROJECT = 'surfjudging_field_prod';
const WEB_PORT = 8080;
const API_PORT = 8000;
const EVENT_ID = 20001;
const HEAT_ID = 'p38-field-prod_open_r1_h1';
...
function runSql(sql) {
  return execSync(`docker exec surfjudging_field_prod_postgres psql -X -v ON_ERROR_STOP=1 -U postgres -d postgres -Atc "${sql.replace(/"/g, '\\"')}"`, {
    encoding: 'utf8',
  }).trim();
}
...
async function runVerification() {
  log('======================================================');
  log('🚀 TEST E2E DU LAUNCHER TERRAIN : surfjudging-field.sh');
  log('======================================================');

  // 1. Initial Launch
  log('Étape 1 : Démarrage du runtime de production via ./surfjudging-field.sh --no-caffeinate');
  execSync('./surfjudging-field.sh --no-caffeinate', { cwd: rootDir, stdio: 'inherit', env: { ...process.env, SURF_FIELD_PROJECT: PROJECT } });
  ...
  execSync(`docker exec surfjudging_field_prod_postgres psql -U postgres -d postgres -c "
  ...
  execSync(`./surfjudging-field.sh --preflight ${EVENT_ID}`, { cwd: rootDir, stdio: 'inherit', env: { ...process.env, SURF_FIELD_PROJECT: PROJECT } });
  log('✓ Préflight compétition 100% PASS');

  // 5. Chromium UI test (Admin + Judge + Priority + Display concurrently on same-origin :8080)
  log('Étape 5 : Test navigateur simultané (Admin + Judge + Priority + Display sur port 8080)...');
  ...
  execSync('./surfjudging-field.sh --stop', { cwd: rootDir, stdio: 'inherit', env: { ...process.env, SURF_FIELD_PROJECT: PROJECT } });
  ...
  execSync(`./surfjudging-field.sh --backup ${EVENT_ID}`, { cwd: rootDir, stdio: 'inherit', env: { ...process.env, SURF_FIELD_PROJECT: PROJECT } });

  const backupDir = path.resolve(process.env.HOME, 'surfjudging-backups');
  const files = fs.readdirSync(backupDir).filter(f => f.startsWith('surfjudging_field_surfjudging_field_prod_event-20001_') && f.endsWith('.dump'));
  if (files.length === 0) {
