...
function ScoreDisplay({ config, scores, timer, configSaved }: ScoreDisplayProps) {
  ...
  const handleSaveConfig = () => {
    // Nettoyer les scores avant de sauvegarder la nouvelle config
    ...
    if (savedScores) {
      try {
        ...
        const filteredScores = parsedScores.filter((s: any) => s.heat_id !== newHeatId);
        localStorage.setItem('surfJudgingScores', JSON.stringify(filteredScores));
        console.log('🧹 ADMIN: Scores nettoyés pour le nouveau heat:', newHeatId);
      } catch (error) {
        console.error('Erreur nettoyage scores admin:', error);
      }
    }
