...
import { saveHeatsToDatabase } from '../lib/supabase';
  ...
  const handleConfirm = async () => {
    try {
      ...
      for (const categoryPreview of previewData) {
        const rounds = categoryPreview.rounds.map(round => ({
          round: round.round,
          heats: round.heats.map(heat => ({
            ...heat,
            division: categoryPreview.category
          }))
        }));
        ...
        const payload = {
          ...
          metadata: {
            ...
            seriesSize: categoryPreview.seriesSize,
            roundFormat,
            category: categoryPreview.category,
            participants: categoryPreview.participants.length,
            createdAt: new Date().toISOString()
          }
        };

        await saveHeatsToDatabase(payload, eventId);
      }
      ...
      const configPayload = {
        competition: competitionName || eventId,
      ...
      alert(
        error instanceof Error
          ? error.message
          : 'Erreur lors de la sauvegarde dans la base de données. Veuillez réessayer.'
      );
    }
  };
                              ...
                              <div key={index} className="bg-gray-900 rounded-lg p-4">
                                ...
                                <div className="space-y-2">
                                  {heat.surfers.map((surfer, idx) => (
                                    <div
                                      key={idx}
                                      className={`p-3 rounded-lg grid grid-cols-[auto,1fr] gap-3 items-center ${
                                        surfer.color === 'ROUGE'
                                          ? 'bg-red-600'
                                          : surfer.color === 'BLANC'
