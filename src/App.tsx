...
function ScoreDisplay({ config, scores, timer, configSaved }: ScoreDisplayProps) {
  ...
  useEffect(() => {
    if (!configSaved || !config.competition) return;
      ...
      // Effacer les scores de l'ancien heat si on change de configuration
      const newHeatId = `${config.competition}_${config.division}_R${config.round}_H${config.heatId}`;
      const savedScores = localStorage.getItem('surfJudgingScores');
      
      if (savedScores) {
        try {
          const parsedScores = JSON.parse(savedScores);
          // Garder seulement les scores qui ne sont pas du nouveau heat
          const filteredScores = parsedScores.filter((s: Score) => s.heat_id !== newHeatId);
          localStorage.setItem('surfJudgingScores', JSON.stringify(filteredScores));
          setScores(filteredScores);
          console.log('🧹 Scores nettoyés pour le nouveau heat:', newHeatId);
        } catch (error) {
          console.error('Erreur nettoyage scores:', error);
        }
      }
      
    const interval = setInterval(async () => {
      const heatId = newHeatId;
