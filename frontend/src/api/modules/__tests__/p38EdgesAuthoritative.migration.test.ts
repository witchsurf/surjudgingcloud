...
describe('P3.8 authoritative progression edges from-zero contract', () => {
  it('ships the edge-aware migration as the final required manifest migration with its exact SHA-256', () => {
    ...
    expect(Math.max(...manifest.migrations.map((migration) => migration.order))).toBe(12);
  });
